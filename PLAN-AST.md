...
### Phase 3 — Runtime-wired ports (`strInput01` via guipropmaps) · *shipped 2026-05-06*

Implementation notes (post-merge):
- Generalized from "just `strInput01`" to "every dict-keyed config read inside a code-component class" — the high-leverage set turned out to be the *config* keys (`strSignalCond`, `strInteractionAlarm`, `strCondMonitor01`, `strCTClear`) which are populated in data, not `strInput01` itself which is empty across all 6 of its occurrences. Phase 3's value-add: making `code-emitted conditions` (`IsReadyPressureSense`, `IsReadyPumpAir`, `IsReadyHeat`) navigable from the guipropmaps that point at them. 17 of 38 RuntimeWiresTo edges dangle exactly because those targets aren't materialized in `data/conditions/`.
- Added a receiver allow-list for dict-key reads (`dictionary` / `gpm` / `*propmap*` / `*mapgpm*`) to filter out unrelated dictionaries — crucially, this excludes `mapGUIPropMaps` (the OUTER per-CO lookup whose keys are CO ids, not config-port keys) and `mapInfo` / `mapGasMols`.
- Local-variable trace bounded to the nearest enclosing `IfStatement` / `Block` to prevent cross-key contamination when an `out string text` is reused across multiple TryGetValue calls (GasPump.SetData reads strInput01, bTurbo, bReverse, bSlowMode, nKnobBus all into the same `text`; only strInput01 flows into `GetCOByID(text)`).
- Three new consumer shapes added to the typing-endpoint set: `co.HasCond`/`AddCondAmount`/`RemoveCond`/etc. → `conditions/`, `ship.GetCOByID` → `condowners/`. The existing `DataHandler.Get*` map carries forward.
- Existence check intentionally NOT applied to RuntimeWiresTo edges — the whole point is surfacing code-emitted refs that wouldn't otherwise have a target node. Site renders them as dangling, which is the correct UX.
- See [DEV-LOG 2026-05-06](DEV-LOG.md) for live build numbers and acceptance smoke.
...
### Phase 3.1 — Polish: synthesize code-emitted conditions, fill the remaining untyped ports, refactor the resolver onto a def-use map · *proposed 2026-05-06*

Same complexity tier as Phases 1-3 (AST-only, no `SemanticModel`). Four
loosely-coupled deliverables that share the resolver code path, none of
which need a full new architecture rung. Ship in order; each is small.

#### A — Promote code-emitted conditions to first-class graph nodes

Phase 3 surfaced 17 RuntimeWiresTo edges to dangling targets like
`conditions:IsReadyPressureSense`, `conditions:IsReadyPumpAir`,
`conditions:IsReadyHeat`. Modders can now SEE them, but clicking through
lands on a *"No object known"* placeholder. The hop-from-modder stops
short.

The fix: synthesize the missing condition nodes from the runtime-port
discovery pass. For every `(component, runtime-key, value)` triple that
emits a RuntimeWiresTo to a folder/strName not already in
`existenceSet`, create a new synthetic `DataObject` in `conditions/`
with a `kind: "code-emitted"` flag in its `Fields`. The detail page then
renders a dedicated layout: *"This condition isn't defined in
`data/conditions/` — `code-component:GasPressureSense` sets / clears it
at runtime via `co.AddCondAmount(this.strSignalCond, …)` driven by the
`strSignalCond` value of the wiring guipropmap."* Includes the seven
guipropmap entries that point at `IsReadyPressureSense` as
incoming-edge sources, plus the producing component as an outgoing-by-
construction link.

Implementation cost: ~80 LOC in `Program.BridgePhase2` (collect
candidates during the existing RuntimeWiresTo loop, synthesize after).
Site work: a small branch in `renderObjectDetail` to check for
`fields.kind === 'code-emitted'` and render the alternate header.

Acceptance: clicking *"IsReadyPressureSense"* from any of the
`AlarmPressure*` guipropmap entries lands on a populated detail page
(not the *"No object known"* placeholder). The page enumerates all
seven guipropmaps that name it and links to its producing
`code-component:GasPressureSense`.

#### B — Per-component helper-class follow-into for `Destructable`

Of 14 dispatcher commands, `Destructable` is the only one with a typed
in-port the resolver can't currently reach: the dispatcher does
`destructable.SetData(array)`, `Destructable.SetData(string[] aStrings)`
forwards to `DestCheck.SetData(aStrings, this.co)` on a *different*
class, and that's where the indices get typed (aStrings[1] = damage
condition, aStrings[2] = action interaction, aStrings[3] = max-cond,
aStrings[4] = numeric).

The Phase 2.1 resolver caps follow-into at the impl type itself; this
deferred case calls for a small allow-list on the impl type that
extends the lookup to specific helper classes. New `CodeComponent`
field — `FollowIntoTypes: IReadOnlyList<string>` — defaulted to empty
for every existing component, populated as `["DestCheck"]` for
`Destructable`. The whole-array forwarding pass already in
`ResolvePortFromArray` then does its `typesByName` lookup against the
union of `[implType] ∪ FollowIntoTypes`.

How the allow-list gets populated: hardcoded for now (one entry, plain
fact about the decomp). If a future component grows a similar second
hop, add a line. PLAN-AST's "resolution rules live in the indexer"
philosophy continues — this is a micro-rule, not a new architecture
layer. Acceptance: `Destructable` shows `[1] → conditions`,
`[2] → interactions`, `[3] → conditions` typed in-ports; ~963 dispatcher
data instances each gain three typed-arg WiresTo edges.

#### C — Container-grouping inside `renderCodeRefsBlock`

Phase 1.1 collapsed sibling literals on `code-method` / `code-class`
detail pages (the *"`new[] { … }` [lines X–Y]"* blocks). The same
treatment doesn't apply yet to the *Code references (N)* block on
data-side detail pages — when one method touches the same condition
multiple times (e.g. `GasPump.Pump` reads `IsOverrideOn` at line 187
and again at line 397), the conditions detail page lists each occurrence
separately under one source group, which is fine, but when one method
references multiple condition LITERALS in a single array initializer
(rare but real in `CondOwner`-style classes), each gets its own row.

Mirror Phase 1.1's grouping logic in `renderCodeRefsBlock`: bucket
incoming edges per source by `metadata.containerKey` first, source-
position second. Existing data is sufficient (Phase 1.1 already populates
`containerKey` on every literal edge). ~30 LOC in `app.js`.

Acceptance: `conditions/Placeholder` detail page (referenced by a 4-element
array literal in `CrewSim`, lines 8932–8937) renders one block with
both literals listed, not two adjacent rows.

#### D — Refactor the resolver onto a per-method def-use map

Currently the resolver chases values forward: when it sees
`this.field = paramName`, it walks the impl type for `this.field` uses.
When it sees `out var local`, it walks the *enclosing if-statement*
(scope-bounded to dodge the GasPump.SetData bug where one `out text`
local serves five TryGetValue calls).

Symbol-assignment tracking IS doable on the AST alone — every
assignment is an `AssignmentExpressionSyntax` and every parameter
binding is a parameter declaration in the surrounding method. Roslyn
exposes both syntactically; we don't need `SemanticModel` for this.
What we'd build is a per-method def map: a list, ordered by source
position, of every `(target-symbol, rvalue-expression)` pair within the
method body. A "use" of a symbol then maps to the nearest preceding
assignment via binary search on source position — proper SSA-lite
without inventing a new IR.

Why we use rvalue + scope-bounded forward-trace today: it was simpler
to ship per-pattern. The hack works for the patterns in this codebase
because each pattern's "scope" is small. The reasons to upgrade:

1. **No more scope-bounding hacks.** The `NearestLocalScope` helper
   exists only because `out text` is reused. A def map handles
   reassignment cleanly: a use of `text` resolves to the most recent
   prior assignment, full stop, no syntactic gymnastics.
2. **Multi-hop alias chains.** `text = dict["K"]; localY = text;`
   `DataHandler.GetX(localY)` should resolve through both hops.
   Today the second hop fails because the resolver doesn't walk an
   intermediate alias.
3. **Future patterns.** Phase 4 (or any later analysis) gets a
   reusable infrastructure piece instead of yet another bespoke
   tracer.

What it costs: ~150 LOC across `ComponentIndexer`. Build a
`MethodDefMap` once per method that owns a relevant pattern (cached on
the method symbol — well, syntactic identity, since we don't have
symbols). Replace the existing forward-walks (`TraceVariableInScope`,
`TraceFieldInClass`, `ResolveParamInMethod`) with queries against the
map. The five existing patterns reduce to "find rvalue at this use
site → recurse with the rvalue." Drops the `originatingCall` /
`scope` parameter threading.

What we *don't* get from this:
- Cross-method tracking (still depth-1 follow-into; def maps are
  per-method).
- Field-write tracking across methods (still walks the type for any
  `this.field = …` site, since fields cross method boundaries by
  definition; def map is the within-method layer).
- Conditional / branch-aware reasoning (a use after `if` / `else` that
  assigns the same symbol differently in each arm picks the
  syntactically-nearest preceding write, which may be wrong; we accept
  this approximation, same as today).

Ship D *last* of the four — it's the largest refactor and the
visibility wins (A, B, C) are independent of it. After D lands, A's
"this.strSignalCond is what GasPressureSense.Run feeds into
co.AddCondAmount" attribution becomes a one-liner instead of the
purpose-built field-walk it'd be without the map.

#### Numbers we expect

```
Phase 3.1 contributions on top of Phase 3:
  + ~17 synthesized conditions/* nodes (code-emitted)
  + ~2,889 WiresTo edges for Destructable (963 instances × 3 typed args)
  +   17 RuntimeWiresTo retargeted from dangling to known nodes
        (no count change, but 0 dangling targets where they used to dangle)
```

Total references after Phase 3.1: ~88,000 (rough estimate; most growth
from Destructable). All four sub-deliverables ship independently —
A, B, C are commits-able on their own; D is a refactor that touches
multiple resolver call sites at once.
...
| 3 | Same shape as Phase 2, extended to `dict.TryGetValue("KEY", out X)` / `dict["KEY"]` patterns + a new `RuntimeWiresTo` edge kind for static-default guipropmap values | `SyntaxTree`, AST-only — `SemanticModel` rung still unnecessary in practice | shipped 2026-05-06 |
| 3.1 | Polish: synthesize code-emitted condition nodes, helper-class follow-into for `Destructable`, container-grouping in `renderCodeRefsBlock`, refactor resolver onto a per-method def-use map | AST-only; def-use is syntactic, not semantic | proposed |
