...
| `build_ship_inspector_data.py` | Extracts the static-site ship-inspector tool's data feeds: per-ship stripped JSON in `src/Ostranauts.Site/data/canned-ships/`, plus `canned-ships-manifest.json` (dropdown driver) and `id-friendly-names.json` (full base-game `strName → strNameFriendly` map). Walks the five `Random*` ship loot tables to enumerate canned regs, follows `strCOBase` chains to resolve each item's `aStartingConds`, and classifies each into one of eight buckets (walls / floors / doors / conduits / containers / equipment / decorative / other). Outputs are checked in; re-run via `make ship-inspector-data` when source data shifts. |
