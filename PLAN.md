...
| [mod-npc-vendor](notes/user-stories/mod-npc-vendor.md) | EXPLORER | Response-menu framing banner on `aInverse` + per-row gate pill summarising target's `CTTestUs` / `CTTestThem`; derived "How is this granted?" section on Condition detail pages, broken out into static-template vs. plot-driven grants; schema overlay enrichment for `aInverse` (fold the base-schema's chat → chat-reply example + `,[us],[them]` role-suppressor mechanic into our gloss). |
